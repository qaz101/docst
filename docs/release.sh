...
download_v2ray_core(){
  ...
  # 下载最新amd4版本V2ray
  arm64_link="${V2RAY_CORE_RELEASE_URL}/download/${latest_ver}/v2ray-linux-64.zip"
  curl "${arm64_link}" -k -L -o ./tmp/v2ray-core-amd64.zip >&2
  if [ "$?" != "0" ] ; then
    echo "Error: Download V2Ray core[amd64] failed."
    exit 1
  fi
...
zip_files(){
  ...
  # 创建64位amd版本zip文件
  zip -q -r v2ray-magisk-androidx64.zip META-INF v2ray customize.sh README.md service.sh uninstall.sh
  ...
  cp tmp/geoip.dat v2ray/bin/geoip.dat
  cp tmp/geosite.dat v2ray/bin/geosite.dat
  unzip -j -o tmp/v2ray-core-amd64.zip "v2ray" -d v2ray/bin/
  cp module.prop.bk module.prop
  zip -q -r -u v2ray-magisk-androidx64.zip v2ray/bin module.prop
